...
export async function fetchPlaylistById({ id, limit = 10 }) {
  ...
  if (DISABLE_NETWORK_REQUESTS) {
    return { error: "Network disabled and no cached playlist available" };
  }
...
// Update likedAt timestamp for offline song
export async function updateOfflineSongLikedAt(songId, likedAt) {
  if (typeof window === 'undefined' || !songId) {
    console.log('❌ updateOfflineSongLikedAt: Invalid parameters', { songId, likedAt });
    return false;
  }
  
  console.log('🔄 updateOfflineSongLikedAt called:', { songId, likedAt });
  
  try {
    const db = await initOfflineAudioDB();
    if (!db) {
      console.log('❌ updateOfflineSongLikedAt: Failed to init DB');
      return false;
    }
    
    const transaction = db.transaction([OFFLINE_METADATA_STORE], 'readwrite');
    const store = transaction.objectStore(OFFLINE_METADATA_STORE);
    ...
    console.log('📋 Existing data for song:', songId, existingData);
    
    if (existingData) {
      // Update the likedAt timestamp
      const updatedData = {
        ...existingData,
      ...
      console.log('✅ Updated likedAt for offline song:', songId, 'to:', updatedData.likedAt);
      return true;
    } else {
      console.log('❌ No existing data found for song:', songId);
    }
    
    return false;
  } catch (error) {
    console.error('❌ Error updating offline song likedAt:', error);
    return false;
  }
}
